...
solo <- randomForest(winPlacePerc ~ ., data = head(trainsetSolo, 10000), na.action = na.omit, importance=T, keep.inbag = T)})
solo
```
...
On voit que ça semble ce stabiliser vers 300. On va donc utiliser cette valeur pour ntreeTry.  
...
solo <- randomForest(winPlacePerc ~ ., data = head(trainsetSolo, 10000), na.action = na.omit, importance=T, mtry=10, ntree=300)})
solo
```
